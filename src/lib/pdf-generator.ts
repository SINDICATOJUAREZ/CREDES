...
// Vectorial Generator
// Map DB design to CredentialConfig interface
export const mapDesignToConfig = (design: any): CredentialConfig => {
  return {
    id: design.id,
    name: design.name,
    primaryColor: design.primary_color || design.primaryColor || '#003366',
    secondaryColor: design.secondary_color || design.secondaryColor || '#EAB308',
    backgroundUrl: design.background_url || design.backgroundUrl || undefined,
    showTemplate: design.show_template !== false && design.show_template !== 0 && design.show_template !== 'false',
    elements: (design.elements || []).map((el: any) => ({
      id: el.id,
      label: el.label,
      field: el.campo_bd || el.field,
      type: el.tipo === 'qr' ? 'qr' : el.tipo === 'imagen' || el.tipo === 'image' ? 'image' : 'text',
      x: el.x || 0,
      y: el.y || 0,
      w: el.w,
      h: el.h,
      fontSize: el.font_size || el.fontSize || 8,
      fontWeight: el.font_weight || el.fontWeight || 'bold',
      color: el.color || '#000000',
      isVisible: el.is_visible !== false && el.is_visible !== 0,
      fixedText: el.fixed_text || el.fixedText || undefined,
...
export const generateResumePDF = async (member: Member) => {
  ...
  try {
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.text('Puesto Actual:', 20, 106);
    pdf.setFont('helvetica', 'normal');
    pdf.text(member.position || '---', 55, 106);
    
    pdf.setFont('helvetica', 'bold');
    pdf.text('Departamento:', 20, 113);
    pdf.setFont('helvetica', 'normal');
    pdf.text(member.department || '---', 55, 113);
    
    // Section: DATOS DE CONTACTO
    pdf.setTextColor(0, 51, 102);
    pdf.setFontSize(13);
    pdf.setFont('helvetica', 'bold');
    pdf.text('DATOS DE CONTACTO', 20, 126);
    pdf.setDrawColor(0, 51, 102);
    pdf.setLineWidth(0.5);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.text('Teléfono:', 20, 136);
    pdf.setFont('helvetica', 'normal');
    pdf.text(member.phone || '---', 55, 136);
    
    pdf.setFont('helvetica', 'bold');
    pdf.text('Email:', 20, 143);
    pdf.setFont('helvetica', 'normal');
    pdf.text(member.email || '---', 55, 143);
    
    pdf.setFont('helvetica', 'bold');
    pdf.text('Domicilio:', 20, 150);
    pdf.setFont('helvetica', 'normal');
    pdf.text(member.address || '---', 55, 150);
